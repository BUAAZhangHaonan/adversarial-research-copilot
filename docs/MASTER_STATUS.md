...
真实总验收仍未通过。旧discover完成一张MAIN_REPORT卡，旧develop/run及三组质量对照未完成。当前已启动自然同卡新运行版本 `arc-vnext-validation-20260907.natural_tool_correction_v1.develop`，运行源码为21b0cd2；只有develop完成后才选择其新版本进入run。旧失败原件不变，不把工程测试通过等同于端到端通过。
...
本轮复验开始前累计估计45.154048–45.303543元，原100元父预算剩余54.696457元。此前最终停点是协议错误，不是费用不足，因此不抹除旧账或重置100元；本轮新的调用继续计入该父账本。
...
主仓库完整离线测试370项通过（79.86秒）；独立wheel安装检查确认新模板随包加载。自然同卡的复验正在使用原100元父预算，后续以实际停点更新本文件。64000字符的容量试验是已评估的可选支线，尚未执行；建议先完成当前24000字符下的纠正验证，再按测得收益决定是否提高。
