...
日期：2026-09-09。以本轮 `ARC_Discover_First_CodeX_Execution_2026-09-09.md` 及配套 Markdown 为需求依据。代码接线已落地；最终软件、真实案例与费用记录由主任务完成后填写，本页不提前宣称真实验收完成。
...
| 检查 | 当前记录 |
| --- | --- |
| 默认轻量流、共享材料、早筛不调用CHECK、额度恢复 | 待主任务汇总最终相关回归 |
| --idea 人工交接与当前阶段状态 | 待主任务汇总最终离线验证 |
| 模型别名、供应商边界及Markdown打包资源 | 待主任务汇总最终检查与包路径 |
| 必要整体回归与安装 | 待主任务填写实际结果，不以测试数量代表研究能力 |
| 开发例1：多模态模型在真实任务中何时需要主动获取额外信息 | 待执行/汇总；20元上限 |
| 开发例2：具身智能在环境变化后如何使用和更新既有经验 | 待执行/汇总；20元上限 |
| 本轮实际费用与原父账本剩余额度 | 待读取真实账本后填写，不引用上轮余额代替 |

真实验收最多两个discover，各20元、累计至多40元且不超过现有已授权父账本余额；这是进一步限制，不是新增授权。不要清零花费、提额凑五次、串行追加模型/流程对照或自动付费跑develop/run。最多因实际暴露的一个主要流程/提示词问题调整一轮，追加真实复核仍计入同一40元。
