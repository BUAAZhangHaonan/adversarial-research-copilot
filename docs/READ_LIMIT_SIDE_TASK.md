...
2026-09-07 更新：用户已选择提高到 64000 字符并完成分页、预算和恢复验证。2e152b4 已实现并启用公开上限；默认仍为 12000 字符。实际 read_record/Runtime 回归覆盖 Unicode 分页、64001 拒绝、一次工具参数纠正、预算暂停及重建 Runtime 后恢复不重放。没有改变 max_tokens、模型和预算上限，也未声称已证明质量或费用收益。

此前未启用是开发安排上的保留选项，不是已经发现技术障碍；这一决定已被用户的明确选择替代。本项不再等待用户创建支线。当前结果见 [MASTER_STATUS.md](MASTER_STATUS.md)。

下方保留原需求与旧评估作为历史记录。其中“当前上限 24000”“尚未执行”和 CodeX 默认评估模板不描述当前版本。
