...
并进入 `pending_codex_review`。科研报告的 `MCP_REQUIREMENTS.md` 汇总需求与评估。
...
2. 用 `arc --data-dir .arc requirements export REQUEST_ID --output review-handoff.md`
   导出 CodeX 评估模板。用户可把它交给 CodeX；命令本身不创建任务或调用模型。
3. CodeX 查阅相关实现后，写出以下结构的评估 JSON，再用
...
```json
{
  ...
  "rationale": "说明已核对的事实、方案取舍及尚未确认的影响。",
