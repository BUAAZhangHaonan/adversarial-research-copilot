...
截至 2026-09-08，核心构思、独立科学审查、实际修订与复核已接通。四组八个真实语义对照完成目标纠错验收；这不等于科研品味或新 idea 质量已整体提高。新主题 draw1 完成一次科学闭环但仍为 NEEDS_EVIDENCE，draw2 仅交付初稿；自然流程后续、A–E 比较和旧卡定向纠正尚未完成。具体结果与费用见[验收结果](FUNCTIONAL_VALIDATION_RESULTS.md)。
...
| §2.1：原题、实质条件持续可见 | `research_context.build_research_context()`、统一 `context()`；原题跨修订/恢复测试 | 原题、用户 boundaries、FRAME mandate 与提案来源分别呈现；v2 冲突总体组实际修复偏题。模型首稿的方法细节不冒充用户要求 |
| §2.1、§14：L4 输入一致 | `evaluation.py` 明确 original_task，并传 FRAME mandate 至 NEXT_DRAW/COMPOSE 与审核 | 输入传递测试通过；三个旧主题仍是开发回归，不能重新当未见留出 |
| §2.2：查错必须改正文再复核 | `scientific.py`、CONCEIVE/REVISE、ScientificReview/ScientificRevision；科学闭环测试 | v2 四个错误例实际保存修订，四个正确例无目标硬错误误报；新主题 draw1 也完成初审、修订与复核，但新增缺陷未消失 |
...
| §6：调查形成关系判断，支持按需深读 | `roles/investigator.md`、curated excerpt/locator、`read_record` | 新主题四篇冻结一手全文材料；draw2 实际以 64000 字符上限连续读取同源两页，并定向查近邻。该 trace 仅证明这次分页使用，不替代全部预算/恢复测试 |
| §7：先核心认识，再推最小检验 | discovery CONCEIVE 与当前分步提示词；机制、边界、简洁方法的校准例子 | draw1 提出充分性分数与答案可恢复性的边界错位；draw2 初稿探索漂移的文本可检测性。候选本身还不是经验证的洞察 |
...
| §14 最后：新主题自然全流程 | `start_natural_validation.py` 分步建立新 campaign，不把 material_only 当生成结果 | draw1 完整 cycle、draw2 初稿；后续选卡、develop/run、最终验收均待完成。旧三个主题和旧 v7 都不称未见主题 |
