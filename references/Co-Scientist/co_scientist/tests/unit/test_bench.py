...
def test_every_preset_names_a_real_backend() -> None:
    """Candidates and judges must name a backend `get_provider` can construct."""
    from co_scientist.llm.provider import KNOWN_PROVIDERS

    for preset in PRESETS.values():
        for c in preset.candidates:
            assert c.provider in KNOWN_PROVIDERS, (
                f"{preset.name}/{c.label} names unknown provider {c.provider!r}"
            )
        backend, _, _model = preset.suggested_judge.partition(":")
        assert backend in KNOWN_PROVIDERS, (
            f"{preset.name} judge names unknown provider {backend!r}"
        )


def test_no_preset_mixes_backend_families() -> None:
    """A preset is one Elo tournament, so its candidates must be comparable.

    Under a CLI backend the harness delegates the tool loop to the CLI instead
    of driving it, so an API candidate and a CLI candidate in the same
    tournament would be scored against different harnesses, not different
    models. The judge has to sit in the same family for the same reason.
    """
    from co_scientist.llm.provider import CLI_BACKENDS

    for preset in PRESETS.values():
        families = {c.provider in CLI_BACKENDS for c in preset.candidates}
        assert len(families) == 1, (
            f"{preset.name} mixes API and CLI candidates: "
            f"{sorted({c.provider for c in preset.candidates})}"
        )
        judge_backend, _, _model = preset.suggested_judge.partition(":")
        assert (judge_backend in CLI_BACKENDS) == families.pop(), (
            f"{preset.name} judge {judge_backend!r} is not in the candidates' family"
        )
