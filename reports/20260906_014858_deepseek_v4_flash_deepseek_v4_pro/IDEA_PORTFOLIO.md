...
## I1 — When end-to-end failure attribution says planning and instruction errors dominate, what fraction of those errors would disappear if an otherwise identical agent had oracle-perfect memory at every decision point?


- from gaps: ['G4']
- evidence: [2604.11978] attributes most long-horizon failures to planning, with memory limitation a minority; [2608.15008] finds memory retrieval quality does not transfer to ALFWorld planning success; [2606.24775] finds a no-memory long-context baseline wins DB-Bench EM.
...
## I2 — When retrieval quality improves without improving end-to-end success, at which subgoal does good recall fail to change behavior — recalling the fact, using it to update state, or selecting the next action?


- from gaps: ['G4']
- evidence: [2608.15008] shows retrieval quality transfers poorly to ALFWorld planning success; [2604.11978] shows planning errors dominate end-to-end failures; the pool contains no checkpoint-level decomposition of where good retrieval stops helping.

- who needs it: Retrieval-benchmark authors who currently optimize memory EM as a proxy for agent success, and planning researchers needing to know where the handoff breaks.

- why now: Long-horizon runs are now logged with per-subgoal world-state annotations, enabling retrieval-correctness and next-action-correctness to be scored at the same checkpoint rather than only at final episode success.

- minimal falsifiable test: On an ALFWorld-style suite, annotate each subgoal with (i) the fact that must
be recalled and (ii) the action chosen after recall. Condition on correct recall
and measure the probability the next action is correct. If it is below 50%, good
recall is not the binding constraint at that point; if it is high, failures
concentrate in the recall step itself.

- anti-scope: Not a new retrieval model, reranking method, or a claim that retrieval is useless; not a full benchmark comparison.


## I3 — Which property of a long-horizon task — raw trajectory length, number of distinct subgoals, or the distance between an early fact and its late use — actually determines when a no-memory long-context baseline loses to a memory-augmented agent?


- from gaps: ['G4']
- evidence: [2606.24775] finds a no-memory long-context baseline wins DB-Bench EM; [2604.11978] reports memory limitation is only a minority of long-horizon failures; no pool paper orthogonally varies these horizon properties.
...
## I4 — Is the advantage of a memory architecture over a full-transcript no-memory agent concentrated in multi-target interference settings, rather than in long-horizon tasks generally?


- from gaps: ['G4']
- evidence: [2605.18565] constructs MINTEval around multi-target interference as the memory stressor; [2606.24775] finds no-memory long-context beats memory on DB-Bench; [2608.15008] finds retrieval gains do not transfer to ALFWorld planning success.
...
## I5 — In long-horizon agents that fail to obey early instructions, is the failure a memory-storage deficit, or is it a context-attention deficit that a trivial stepwise instruction-reminder baseline eliminates without any memory architecture change?


- from gaps: ['G4']
- evidence: [2604.11978] attributes many long-horizon failures to instruction-following and planning errors rather than memory; memory papers in the pool (e.g., [2606.06448], [2601.09913]) treat holding instructions across long horizons as a memory problem.
...
## I6 — If planning capability is sharply increased via a reasoning-tuned LLM, does the remaining long-horizon failure mix shift toward memory errors — making memory the true bottleneck only after planning is solved?


- from gaps: ['G4']
- evidence: [2604.11978] reports planning errors dominate long-horizon failures; [2501.12948] shows RL reasoning training can improve planning/reasoning; no pool paper asks whether the memory-vs-planning failure share changes after such a planner upgrade.
...
## I7 — At equal added inference cost, which intervention cluster buys the largest end-to-end success gain on the same long-horizon suite: a memory substrate, a planning wrapper, or instruction/context engineering?


- from gaps: ['G4']
- evidence: [2608.15008] memory-substrate gains do not propagate to ALFWorld planning success; [2606.24775] a no-memory long-context baseline wins DB-Bench EM; [2604.11978] planning errors dominate; the pool lacks any matched-cost comparison across clusters.
...
## I8 — When a full-transcript no-memory long-context baseline already beats memory agents, is the active ingredient full information-in-context, and under hard context limits does selective memory outperform a random-truncation baseline?


- from gaps: ['G4']
- evidence: [2606.24775] finds no-memory long-context wins DB-Bench EM; [2605.30785] learns context management; [2601.09913] and [2601.06377] propose memory architectures; no pool ablation separates the value of information availability from the value of learned selection.
