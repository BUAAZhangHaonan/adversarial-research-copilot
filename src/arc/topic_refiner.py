...
def refine_research_topic(
    ...
    for rid in range(1, max(1, rounds) + 1):
        ...
        critique_output = client.chat(
            ...
            user_prompt=_reviewer_prompt(refined),
            temperature=0.2,
        )
...
def _reviewer_system_prompt() -> str:
    return (
        "你是严苛审稿人。你要找出题面中的不可验证、不可执行、不可复现部分，"
        "并给出最小修改动作。"
    )


def _writer_prompt(topic: str, previous_critique: str | None) -> str:
    critique = previous_critique.strip() if previous_critique else "无"
    return (
        "请将以下研究问题优化为可执行题面。\n"
        "要求：\n"
        "1) 一句话题目（20-60字）\n"
        ...
        "5) 资源预算上限\n"
        "请优先输出标题 '# 优化题面'，下一行给最终一句话题目。\n\n"
        f"原始问题：\n{topic}\n\n"
        f"上一轮审稿意见：\n{critique}\n"
    )


def _reviewer_prompt(refined: str) -> str:
    return (
        "请审查下述优化题面是否具备可执行性。\n"
        "输出四段：\n"
        "1) 关键缺陷\n"
