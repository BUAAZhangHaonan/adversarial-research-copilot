...
If an explicit task revision reports `problem_anchor_changed`, reproduce the supplied `expected` problem_anchor exactly. Put valid implementation, measurement, and preregistration refinements in the corresponding method, minimal_test, claim, or limitation fields instead. Do not change the research question or silently broaden its frozen conditions. The earlier schema-valid proposal was rejected at application time and is not an applied card revision.
