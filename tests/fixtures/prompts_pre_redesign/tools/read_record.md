...
The maximum limit is 64000 characters, not model tokens. Request only the range needed for the decision. Use offset pagination for longer cached text. Increasing the maximum does not prove that a whole source was read or increase the model output/budget allowance.
