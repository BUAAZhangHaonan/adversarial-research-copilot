...
If an explicit task revision reports `problem_anchor_changed`, reproduce the supplied `expected` problem_anchor exactly. Put valid implementation, measurement, and preregistration refinements in the corresponding method, minimal_test, claim, or limitation fields instead. Preserve the original user question and its essential conditions. The explicit diagnostic constrains this correction only; it does not turn model-chosen method parameters into permanent user requirements. The earlier schema-valid proposal was rejected at application time and is not an applied card revision.
